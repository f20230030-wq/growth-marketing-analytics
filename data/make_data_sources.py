...
for label, f in files.items():
    df = pd.read_csv(p + f)
    counts += f"  - {label}: {len(df):,} rows, {df.shape[1]} columns\n"
...
- **Rows (verified):**
{counts}- **Join keys:** mql_id (MQL -> deal), seller_id (deal -> order items), order_id, customer_id; use customer_unique_id for repeat customers
...
"""
open("data/data_sources.md", "w").write(doc)
